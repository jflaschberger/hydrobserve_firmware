...
{
    CyRamVectors[CYINT_IRQ_BASE + ISR_hall__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + ISR_hall__INTC_NUMBER];
}
...
void ISR_hall_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((ISR_hall__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *ISR_hall_INTC_PRIOR = (*ISR_hall_INTC_PRIOR & (uint32)(~ISR_hall__INTC_PRIOR_MASK)) |
...
uint8 ISR_hall_GetPriority(void)
{
    ...
    priority = (*ISR_hall_INTC_PRIOR & ISR_hall__INTC_PRIOR_MASK) >> priorityOffset;
